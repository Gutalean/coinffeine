...
class TomP2PSession(delegate: Peer, override val brokerId: PeerId)
  ...
  def send(to: PeerId, payload: Array[Byte]): Unit = { // TODO: remove
    peerById(to)
  ...
  override def connect(peerId: PeerId): Future[Connection] =
    Future.successful(new TomP2PConnection(peerId, this))
}

object TomP2PSession {
  private val Log = LoggerFactory.getLogger(classOf[TomP2PSession])
