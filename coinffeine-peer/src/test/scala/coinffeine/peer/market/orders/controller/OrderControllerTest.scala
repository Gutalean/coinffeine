...
class OrderControllerTest extends UnitTest with MockitoSugar with SampleExchange {
  ...
  "A mutable order" should "compute the required funds to block" in new Fixture {
    order.requiredFunds shouldBe (amounts.bitcoinRequired.buyer, amounts.fiatRequired.buyer)
  }

  it should "start new exchanges" in new Fixture {
    order.fundsBecomeAvailable(blockedFunds)
  ...
  trait Fixture {
    ...
    val order = new OrderController[Euro.type](
      amountsCalculator, CoinffeineUnitTestNetwork, initialOrder, publisher)
