...
class DaemonConfigStoreTest extends UnitTest {

  "A daemon config provider" should "load the configuration file if it exists" in
    withConfigFile("key = value") { configFile =>
      val instance = new ConfigProvider(new DaemonConfigStore(configFile, configFile.getParentFile))
      instance.userConfig.getString("key") shouldBe "value"
    }

  it should "load an empty user configuration otherwise" in {
    val instance = new ConfigProvider(new DaemonConfigStore(new File("/does/not/exists"), new File("/neither")))
